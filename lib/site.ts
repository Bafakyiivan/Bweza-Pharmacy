const configuredUrl = (process.env.NEXT_PUBLIC_SITE_URL || "https://www.bwezapharmacy.org").replace(/\/$/, "");
const canonicalUrl = configuredUrl === "https://bwezapharmacy.org" ? "https://www.bwezapharmacy.org" : configuredUrl;
...
export function emailHref(subject: string, body?: string) {
  ...
  const bodyParameter = body ? `&body=${encodeURIComponent(body)}` : "";
  return `mailto:${site.email}?subject=${encodeURIComponent(subject)}${bodyParameter}`;
}
